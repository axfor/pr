...
func (r *connectionReader) readSingleCommand() (cmd *pb.BaseCommand, headersAndPayload Buffer, err error) {
	...
	if cmdSize+4 < frameSize {
		// 优化：使用 NewBufferWrapper 避免 NewBuffer 的额外内存分配
		// 必须复制数据因为底层 buffer 会被重用
		payloadData := r.buffer.Read(headersAndPayloadSize)
		payloadCopy := make([]byte, len(payloadData))
		copy(payloadCopy, payloadData)
		headersAndPayload = NewBufferWrapper(payloadCopy)
	}
